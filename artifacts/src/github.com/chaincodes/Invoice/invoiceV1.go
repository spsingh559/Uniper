...
// define product request structure
...
// calling Product chain chaincode
f4 := "getProductByID"
prid:=demandRequests.PRID
queryArgs1 := util.ToChaincodeArgs(f4, prid)

productResponse := stub.InvokeChaincode("ProductCC", queryArgs1, "")
if productResponse.Status != shim.OK {
	errStr := fmt.Sprintf("Failed to query chaincode. Got error: %s", productResponse.Payload)
	fmt.Printf(errStr)
	return shim.Error(errStr)
}
productRequests := ProductRequest{}
json.Unmarshal(productResponse.Payload, &productRequests)
fmt.Print("productRequests : ")
fmt.Println(productRequests)
_voulme, errBool :=strconv.ParseFloat(productRequests.Volume, 64)
_price, errBool :=strconv.ParseFloat(productRequests.Transporter_Cost, 64)
_invoice_amount := _voulme * _price
	...
	_total_amount := _invoice_amount * _vat + _invoice_amount
	total_amount := strconv.FormatFloat(_total_amount, 'f', 2, 64)
	invoice_amount :=strconv.FormatFloat(_invoice_amount, 'f', 2, 64)
	shipment_id := args[3]
	// fetch shipment details from shipment_id
	f1 := "getShipmentByID"
	query_Args := util.ToChaincodeArgs(f1, shipment_id)
...
func (t *Invoices) createSupplierInvoice(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	fmt.Println(demandRequests)
	// invoice_amount := demandRequests.Price
// calling get productID from ProductCC
	f4 := "getProductByID"
	prid:=demandRequests.PRID
	queryArgs1 := util.ToChaincodeArgs(f4, prid)
  ...
  // calculating invoice amount
	_voulme, errBool := strconv.ParseFloat(productRequests.Volume, 64)
	_price, errBool := strconv.ParseFloat(productRequests.Supplier_Cost, 64)
	_invoice_amount := _voulme * _price

	// invoice_amount := productRequests.Volume*productRequests.Supplier_Cost
